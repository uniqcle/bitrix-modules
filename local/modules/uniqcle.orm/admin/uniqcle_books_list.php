...
\Bitrix\Main\Loader::includeModule("uniqcle.orm"); // инициализация модуля
...
if ($POST_RIGHT == "D")
	$APPLICATION->AuthForm("Отказано в доступе");
// здесь будет вся серверная обработка и подготовка данных


// Подключим свой ORM-класс
\Bitrix\Main\Loader::includeModule("uniqcle.orm");

// ID таблицы (название)
$sTableID = "uniqcle.book";
...
if ($lAdmin->EditAction() && $POST_RIGHT == "W") {
	...
	foreach ($FIELDS as $ID => $arFields) {
		...
		$DB->StartTransaction();
		$ID = IntVal($ID);
		$elem = new \Uniqcle\ORM\BookTable;
...
if (($arID = $lAdmin->GroupAction()) && $POST_RIGHT == "W") {
	...
	if ($_REQUEST['action_target'] == 'selected') {
		$cData = new \Uniqcle\ORM\BookTable;
	...
	foreach ($arID as $ID)
	{
		...
		switch ($_REQUEST['action']) {
			...
			case "delete":
				@set_time_limit(0);
				$DB->StartTransaction();
				if(!\Uniqcle\ORM\BookTable::Delete($ID)->isSuccess()) {
					$DB->Rollback();
...
$rsData = \Uniqcle\ORM\BookTable::getList(array(
	"order"  => $arOrder,
	"filter" => $arFilter
));
...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	$row->AddViewField(
		"NAME",
		'<a href="uniqcle_books_edit.php?ID=' . $f_ID . '&lang=' . LANG . '">' . $f_NAME . '</a>'
	);
	...
	$arActions[] = array(
		...
		"DEFAULT" => true,
		"TEXT"    => "Редактировать",
		"ACTION"  => $lAdmin->ActionRedirect("uniqcle_books_edit.php?ID=" . $f_ID)
	);
	// Удаление элемента
	if ($POST_RIGHT >= "W")
		$arActions[] = array(
...
$aMenu = array(
	array(
		"TEXT" => "К списку",
		"TITLE"=> "К списку",
		"LINK" => "/bitrix/admin/uniqcle_books_list.php?lang=".LANGUAGE_ID,
		"ICON" => "btn_list",
	),
	array(
		"TEXT" => "Добавить",
		"TITLE"=> "Добавить",
		"LINK" => "/bitrix/admin/uniqcle_books_edit.php?lang=".LANGUAGE_ID,
...
// Установка заголовка
$APPLICATION->SetTitle("Uniqcle модуль ORM книг");
