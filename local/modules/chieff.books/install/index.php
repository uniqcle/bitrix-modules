...
class chieff_books extends CModule {
    ...
    function __construct() {
        ...
        $this->MODULE_DESCRIPTION = Loc::getMessage("CHIEFF_BOOKS_MODULE_DESCRIPTION");

        // Имя партнера создавшего модуль (Выводится информация в списке модулей о человеке или компании, которая создала этот модуль)
        $this->PARTNER_NAME = Loc::getMessage("CHIEFF_BOOKS_PARTNER_NAME");
        $this->PARTNER_URI = Loc::getMessage("CHIEFF_BOOKS_PARTNER_URI");
    ...
    function installDB() {
        ...
        if (!Application::getConnection(\chieff\books\BookTable::getConnectionName())->isTableExists(Base::getInstance("\chieff\books\BookTable")->getDBTableName()))
            Base::getInstance("\chieff\books\BookTable")->createDbTable(); // Если таблицы не существует, то создаем её по ORM сущности
        if (!Application::getConnection(\chieff\books\BookTable::getConnectionName())->isTableExists(Base::getInstance("\chieff\books\AuthorTable")->getDBTableName()))
            Base::getInstance("\chieff\books\AuthorTable")->createDbTable(); // Если таблицы не существует, то создаем её по ORM сущности
    }
    ...
    // \Bitrix\Main\EventManager::getInstance()->addEventHandler('chieff.books', '\chieff\books\Book::OnBeforeAdd', Array('MyClass', 'MyOrmEvent'));
    ...
    function installFiles() {
        ...
        if (!$res)
            $resMsg = Loc::getMessage("CHIEFF_BOOKS_INSTALL_ERROR_FILES_ADM");
        ...
        if (!$res)
            $resMsg = ($resMsg) ? $resMsg . "; " . Loc::getMessage("CHIEFF_BOOKS_INSTALL_ERROR_FILES_COM") : Loc::getMessage("CHIEFF_BOOKS_INSTALL_ERROR_FILES_COM");
        if ($resMsg) {
    ...
    function installAgents() {
        \CAgent::AddAgent(
            "\chieff\books\Agent::superAgent();", // Строка PHP для запуска агента-функции
    ...
    function unInstallDB() {
        ...
        Application::getConnection(\chieff\books\BookTable::getConnectionName())->queryExecute('DROP TABLE IF EXISTS ' . Base::getInstance("\chieff\books\BookTable")->getDBTableName());
        Application::getConnection(\chieff\books\BookTable::getConnectionName())->queryExecute('DROP TABLE IF EXISTS ' . Base::getInstance("\chieff\books\AuthorTable")->getDBTableName());
    ...
    function DoUninstall() {
        ...
        if ($request["step"] < 2) {
            if (file_exists($_SERVER["DOCUMENT_ROOT"] . "/local/modules/chieff.books/install/unstep1.php"))
                $APPLICATION->IncludeAdminFile(Loc::getMessage("CHIEFF_BOOKS_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"] . "/local/modules/chieff.books/install/unstep1.php");
            else
                $APPLICATION->IncludeAdminFile(Loc::getMessage("CHIEFF_BOOKS_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/chieff.books/install/unstep1.php");
        } elseif ($request["step"] == 2) {
            ...
            if (file_exists($_SERVER["DOCUMENT_ROOT"] . "/local/modules/chieff.books/install/unstep2.php"))
                $APPLICATION->IncludeAdminFile(Loc::getMessage("CHIEFF_BOOKS_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"] . "/local/modules/chieff.books/install/unstep2.php");
            else
                $APPLICATION->IncludeAdminFile(Loc::getMessage("CHIEFF_BOOKS_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/chieff.books/install/unstep2.php");
        }
    }
    ...
    function GetModuleRightList() {
        return array(
            ...
            "reference" => Array(
                "[D] " . Loc::getMessage("CHIEFF_BOOKS_DENIED"),
                "[K] " . Loc::getMessage("CHIEFF_BOOKS_READ_COMPONENT"),
                "[S] " . Loc::getMessage("CHIEFF_BOOKS_WRITE_SETTINGS"),
                "[W] " . Loc::getMessage("CHIEFF_BOOKS_FULL"),
            )
        );
    }
