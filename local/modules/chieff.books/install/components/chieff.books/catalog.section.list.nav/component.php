...
$arCustomSort = [];
if (isset($arParams["SORT_BY1"]) && $arParams["SORT_BY1"] && isset($arParams["SORT_ORDER1"]) && $arParams["SORT_ORDER1"])
    $arCustomSort[$arParams["SORT_BY1"]] = $arParams["SORT_ORDER1"];
if (isset($arParams["SORT_BY2"]) && $arParams["SORT_BY2"] && isset($arParams["SORT_ORDER2"]) && $arParams["SORT_ORDER2"])
    $arCustomSort[$arParams["SORT_BY2"]] = $arParams["SORT_ORDER2"];

$arNavParams = array(
    'nPageSize' => $arParams["SHOW_SECTIONS_COUNT"],
    'bShowAll' => $arParams["PAGER_SHOW_ALL"],
);
$curPage = ($_REQUEST["PAGEN_1"]) ? intval(htmlspecialchars(strip_tags($_REQUEST["PAGEN_1"]))) : 1;
$arNavParams['iNumPage'] = $curPage;
...
))
{
    ...
    if($arParams["SECTION_ID"] > 0) {
        ...
        $rsSections = CIBlockSection::GetList($arCustomSort, $arFilter, false, $arSelect, $arNavParams);

        $arNavigation = [$navComponentObject];
        $arNavigation[] = $arParams["PAGER_TITLE"] ?? "Новости";
        $arNavigation[] = $arParams["PAGER_TEMPLATE"] ?? ".default";
        $arResult["NAV_STRING"] = $rsSections->GetPageNavStringEx($arNavigation[0], $arNavigation[1], $arNavigation[2], $arParams["PAGER_SHOW_ALWAYS"]);
    ...
    $arSort = array_merge($arSort, $arCustomSort);
    ...
    //EXECUTE
    $rsSections = CIBlockSection::GetList($arSort, $sectionFilter, false, $arSelect, $arNavParams);
